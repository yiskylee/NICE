...
class TestACL(unittest.TestCase):
    ...
    def test_8(self):
        data = self.load_data(
            n=40, d=2, c=2, name='gaussian', data_type='float')
    ...
    def test_9(self):
        data = self.load_data(
            n=400, d=4, c=2, name='gaussian', data_type='float')
        acl = ACL('float', 'KDAC', 'cpu')
        acl.set_params(sigma=2.0, c=2, q=1, verbose=1,
                       vectorization=1, thresh2=0.01)
        acl.OutputConfigs()
        acl.Fit(data)
        pred = acl.Predict()
    ...
    def test_11(self):
        data = self.load_data(
            n=683, d=9, c=2, name='breast', data_type='double')
        acl = ACL('double', 'KDAC', 'cpu')
    ...
    def test_12(self):
        data = self.load_data(
            n=624, d=27, c=4, name='facial', data_type='double')
        acl = ACL('double', 'KDAC', 'cpu')
